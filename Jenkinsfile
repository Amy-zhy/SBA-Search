    ...
    stage('Build Docker Image') {
      steps {
        ...
        withCredentials([usernamePassword(credentialsId: 'docker-hub', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
          ...
          bat 'docker image build -t %DOCKERHUBNAME%/sbaamysearch .'
          echo 'Image build successfully!'
          echo 'Start pushing image to docker hub...'
          bat 'docker push %DOCKERHUBNAME%/sbaamysearch'
          echo 'Image push successfully!'
          echo 'Start running...'
          bat 'docker run -d -p 8081:8081 --name sba-search %DOCKERHUBNAME%/sbaamysearch'
